...
Sync section: just the other user's email address, persisted via
ui/sync_partner_settings.py. No on/off switch -- Sync is its own button
on the Current Sheet page now, separate from Update, so there's nothing
left for a toggle to gate; Update never touches this at all.
"""
...
class SettingsPage(QWidget):
    def __init__(self):
        ...
        # Where the Current Sheet page's Sync button (and Finalize, when
        # this is set) send their sync mail (see services/sync_service.py
        # + services/outlook_service.py). Persisted via
        # ui/sync_partner_settings.py so it isn't re-typed every session.
        # No on/off switch here anymore -- Sync is its own button now,
        # separate from Update, so there's nothing for a toggle to gate.
        ...
        self._sync_text = QLabel("Other user's email (for Sync/Finalize on Current Sheet)")

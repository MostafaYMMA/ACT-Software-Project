...
class HistoryPage(QWidget):
    """Export controls (last month / custom date range) + a log of every
    export ever produced (name + date), newest first."""
    ...
    def _run_export(self, start, end, default_name):
        ...
        if row_count == 0:
            self.status_label.setText(f"No records between {start} and {end} - nothing exported.")
        else:
            self.status_label.setText(f"Exported {row_count} row(s) ({start} to {end}) to {path}")

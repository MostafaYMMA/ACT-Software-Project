...
def get_current_sheet_rows(project_type=None):
    """
    Every row across all three status tables (Approved/Pending/Rejected)
    for the CURRENT open period -- received since the last finalize/export,
    or everything if nothing's ever been finalized -- as one combined,
    unsorted-by-status list. Backs the Current Sheet page: one live,
    editable view of "the period so far", not split by status the way
    Dashboard is.

    Each row dict carries a "status" key (the label it came from) so a
    caller editing a cell knows which table to route the write to (see
    update_status_record_field, which already takes a status_key/table
    per call -- nothing new needed there).
    """
    period_start = get_last_export_date()
    ...
    try:
        ...
        for status_label, table_name in STATUS_TABLES.items():
            ...
            cursor = conn.execute(
                f'SELECT * FROM "{table_name}"{where_sql} ORDER BY "Date" ASC, subject ASC', params
            )
            columns = [desc[0] for desc in cursor.description]
...
def init_db():
    ...
    for table_name in STATUS_TABLES.values():
        ...
        _ensure_columns(conn, table_name, [
            ...
            # A small preset-palette tag (e.g. "#F44336"/None) set from
            # the Current Sheet page -- purely a visual marker, not read
            # by any export or scan logic.
